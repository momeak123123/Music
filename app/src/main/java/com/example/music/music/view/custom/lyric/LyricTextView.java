package com.example.music.music.view.custom.lyric;
...
public class LyricTextView extends View {
        ...
        if (!hasLyric) {
            ...
            if (mLyricInfo != null && mLyricInfo.songLines != null && mLyricInfo.songLines.size() > 0) {
                ...
                if (mCurrentPlayLine >= mLyricInfo.songLines.size() - 1) {
                    mEndMillis = MusicPlayActivity.wlMusic.getDuration();
                    nextContent = "";
                } else {
    ...
    public void setLyricInfo(LyricInfo lyricInfo) {
        if (lyricInfo != null) {
            ...
            hasLyric = false;
            mDefaultHint = String.valueOf(R.string.lyric_default_hint);
        }
        invalidateView();
    }
